...
## EC2/Fargate vs. Dockerized Lambda
...
The CDK enables a **declarative Infrastructure as Code (IaC)** approach, synthesizing code into AWS CloudFormation templates while providing **reliable state management** and ensuring repeatable deployments with safer updates. Furthermore, its **integrated deployment flow** fits naturally into Nimbus’s CLI-driven workflow, offering safeguards like dependency resolution and automated rollback mechanisms. Finally, utilizing CDK or other IaC tools are considered best-practice when provisioning AWS-specific infrastructure.
...
Nimbus adopted a simpler approach, utilizing a local configuration file named models.json stored within the user's local artifact directory to maintain deployment state. This method was chosen for its alignment with Nimbus's goals:

* **Simplicity:** A local JSON file is easy for developers to understand, inspect, and edit if necessary. The structure is straightforward, minimizing the learning curve.  
